...
export class PurchaseOrderItemDto {
  ...
  @ApiProperty({
    required: false,
    description:
      'Tên sản phẩm (sẽ tự động lấy từ Good nếu có goodId, không cần nhập)',
    example: 'Sản phẩm A',
  })
  @IsOptional()
  ...
  @ApiProperty({
    required: false,
    description:
      'Giá đơn vị (nếu không cung cấp và có goodId, sẽ tự lấy từ Good)',
  })
  @IsOptional()
  @ValidateIf((o) => !o.goodId)
  @IsNumber()
  ...
  @ApiProperty({
    required: false,
    description:
      'Tổng tiền (price * quantity, sẽ được tự động tính nếu không cung cấp)',
  })
  @IsOptional()
  @IsNumber()
  total?: number;
...
export class CreatePurchaseOrderDto {
  ...
  @ApiProperty({
    required: true,
    description:
      'Tổng giá trị đơn hàng trước khi điều chỉnh (nhập từ frontend)',
  })
  @IsNumber()
  @Min(0)
  subtotal: number;
